...
python src/hh_model.py                # run a single HH point neuron
python src/hh_cable.py                # run the HH cable equation (gold standard)
...
python tests/test_event_model.py      # run tests
```
...
├── demos/            # Visualization and demo scripts
├── tests/            # Tests
├── output/           # Generated plots (.png) and PCA basis data (.npz)
├── AGENTS.md         # Coding guidelines & project context
└── SUMMARY.md        # Detailed SSDS project summary
```
...
| `analysis.py` | Spike detection (threshold crossing), FWHM measurement, waveform extraction |
| `hh_model.py` | HH point neuron with scalar and vectorized multi-compartment paths |
| `hh_cable.py` | HH cable equation implementation|
...
| `event_model.py` | `EventPropagator` — the core model: PCA encode/decode, multi-source convergence with LPF fusion, refractory filtering|
...
| `bench_spatial_complexity.py` | O(N) vs O(1) spatial scaling across 50–50,000 compartments |
| `bench_axon_length.py` | Axon length scaling (0.5–100 mm), fixed resolution |
| `bench_scaling.py` | Neuron count scaling (1–50,000 neurons) across 5 model types |
| `bench_full_system.py` | End-to-end latency: generation vs propagation time breakdown |
...
### `demos/` — Visualization and Exploration
...
- `sweep_ais_params.py` — AIS parameter sweep for optimal sharp-AP parameters
- `plot_*.py` — Publication-ready figure generation scripts
